...
const newDataAndLogic = `const data = [
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'Plan 2', price: '600k OwO', features: ['2 vCore', '12GB RAM', '40GB Disk (VPS)', 'Pterodactyl Panel Supported'] },
          { name: 'Plan 3', price: '800k OwO', features: ['3 vCore', '12GB RAM', '50GB Disk (VPS)', 'Pterodactyl Panel Supported'] }
        ]
      },
      {
        ...
        items: [
          ...
          { name: 'Plan B', price: '2 Boosts', features: ['4 vCore', '16GB RAM', '60GB Disk', 'Renew: 2 Boosts'] },
          { name: 'Plan C', price: '3 Boosts', features: ['6 vCore', '24GB RAM', '120GB Disk', 'Renew: 3 Boosts'] },
          { name: 'Plan D', price: '4 Boosts', features: ['8 vCore', '32GB RAM', '240GB Disk', 'Renew: 4 Boosts'] }
        ]
      },
      {
        id: 'vps-paid-intel', icon: 'cpu', title: 'INTEL VPS (Paid)',
        heading: 'INTEL VPS', desc: 'Coming soon. Stay tuned for ultra-fast Intel-powered Virtual Private Servers.',
        isFiat: true,
        items: [
        ] 
      },
    ...
    function renderAll() {
      ...
      const categories = [
        ...
        { id: 'vps', icon: 'cpu', title: 'VPS Plan' },
        { id: 'domains', icon: 'globe', title: 'Domain Plan' },
        { id: 'pterodactyl', icon: 'layout-dashboard', title: 'Pterodactyl Panel' },
        { id: 'discord', icon: 'message-square', title: 'Discord Setup' }
      ];
      ...
      if (activeCategory === 'minecraft' && !activeSubTab) {
        const plans = data.filter(d => d.id.startsWith('minecraft'));
        ...
        lucide.createIcons();
      } else if (activeCategory === 'vps' && !activeSubTab) {
        const plans = data.filter(d => d.id.startsWith('vps'));
        
        html += '<div class="mb-8 max-w-2xl"><h2 class="text-2xl text-white font-semibold flex items-center gap-2 mb-2"><i data-lucide="cpu" class="text-primary h-6 w-6"></i><span>VPS PLANS</span></h2><p class="text-slate-400 text-sm">Select a category below to view our VPS offerings.</p></div>';
        ...
        const renderId = activeSubTab || activeCategory;
        ...
        if (activeCategory === 'minecraft' && activeSubTab) {
          ...
          html += '<button onclick="setActiveSubTab(null)" class="mb-6 px-4 py-2 bg-dark-card/50 hover:bg-dark-border border border-dark-border hover:border-primary/50 text-white rounded text-xs font-bold uppercase tracking-widest flex items-center gap-2 transition-colors"><i data-lucide="arrow-left" class="h-4 w-4"></i> Back to VPS Plans</button>';
        }

        if (activeData.items.length === 0) {
            html += '<div class="mb-8 max-w-2xl"><h2 class="text-2xl text-white font-semibold flex items-center gap-2 mb-2"><i data-lucide="' + activeData.icon + '" class="text-primary h-6 w-6"></i><span>' + activeData.heading + '</span></h2><p class="text-slate-400 text-sm whitespace-pre-wrap">' + activeData.desc + '</p></div>';
            panesContainer.innerHTML = html;
            lucide.createIcons();
            return;
        }
        ...
        activeData.items.forEach(item => {
          ...
          if (activeData.isFiat !== false && typeof item.price === "number") {
              ...
              priceSection = '<div class="flex items-baseline gap-1 mb-6">' +
                  ...
                  '<span class="text-xs text-slate-500 font-medium uppercase tracking-widest">/ ' + (activeData.domainMode ? 'yr' : 'mo') + '</span>' +
                '</div>';
          } else {
              priceSection = '<div class="flex items-baseline gap-1 mb-6">' +
                  '<span class="text-3xl p-1 font-light text-white">' + item.price + '</span>' +
                '</div>';
          }
          ...
          let infoHtml = '';
          if (item.info) {
             infoHtml = '<div class="mt-4 text-xs p-3 bg-dark-bg border border-dark-border rounded text-slate-400 whitespace-pre-wrap">' + item.info + '</div>';
          }
...
fs.writeFileSync('/workspace/services.html', file.substring(0, strStart) + newDataAndLogic + file.substring(strEnd));
