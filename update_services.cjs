    ...
    const data = [
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'Plan 2', price: '1.9M OwO', features: ['1 vCORE', '16GB RAM', '24GB STORAGE', 'DDoS Protection'] },
          ...
          { name: 'Plan 5', price: '5.9M OwO', features: ['3 vCORE', '32GB RAM', '50GB STORAGE', 'DDoS Protection'] }
        ]
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'Infinite Plan', price: 1199, topTier: true, features: ['Unlimited vCore (AMD Epyc)', 'Unlimited DDR5 RAM', 'Unlimited NVMe SSD', 'Priority Support', 'DDoS Protection'] }
        ]
      },
      {
...
let t = s.substring(s.indexOf('const rates = {'), s.indexOf('let activeTab =') + 30);
t = t.substring(0, t.indexOf(';')+1); // gets let activeTab = 'minecraft';

let p1 = s.indexOf('const rates = {');
let p2 = s.indexOf('function setActiveTab(id) {');
s = s.substring(0, p1) + newData + '\n\n    ' + s.substring(p2);

// replace render logic
const renderTargetRegex = /if \(activeData\.id === 'minecraft'\) \{[\s\S]*?if \(activeData\.domainMode\) \{/m;
const renderTargetRepl = `if (activeData.id.startsWith('minecraft')) {
        html += \`
        <!-- Trial Banner -->
        <div class="mb-8 p-6 rounded-xl bg-primary/10 border border-primary/30 backdrop-blur-md flex flex-col sm:flex-row items-center justify-between gap-4 relative overflow-hidden shadow-lg">
          ...
          <div class="relative z-10 flex items-center gap-4">
            <div class="h-12 w-12 rounded-full bg-primary/20 flex items-center justify-center shrink-0">
              <i data-lucide="clock" class="h-6 w-6 text-primary-light"></i>
            </div>
            <div>
              <h3 class="text-xl font-semibold text-white mb-1">24-Hour Trial <span class="ml-2 text-[10px] font-mono bg-primary/20 text-primary-light px-2 py-0.5 rounded border border-primary/30 align-middle">\` + fm(50) + \` Fee</span></h3>
              <p class="text-sm text-slate-300">Not sure yet? Test our high-performance nodes with full access for a small setup fee.</p>
            </div>
          </div>
          <div class="relative z-10 shrink-0">
            <a href="buy.html" class="px-6 py-3 bg-primary/20 hover:bg-primary/30 text-primary-light border border-primary/50 hover:border-primary font-bold text-xs uppercase tracking-widest rounded transition-colors whitespace-nowrap flex items-center gap-2">
              Claim Trial <i data-lucide="arrow-right" class="h-4 w-4"></i>
            </a>
          </div>
        </div>
        \`;
      ...
      html += \`<div class="mb-8 max-w-2xl">
        <h2 class="text-2xl text-white font-semibold flex items-center gap-2 mb-2">
          <i data-lucide="\${activeData.icon}" class="text-primary h-6 w-6"></i>
          <span>\${activeData.heading}</span>
        </h2>
        <p class="text-slate-400 text-sm">\${activeData.desc}</p>
      </div>\`;

      if (activeData.domainMode) {`;

s = s.replace(renderTargetRegex, renderTargetRepl);

// replace else if subOptions
const subOptionsRegex = /\} else if \(activeData\.subOptions\) \{[\s\S]*?\} else \{/m;
s = s.replace(subOptionsRegex, '} else {');

const finalLoopRegex = /activeData\.items\.forEach\(item => \{[\s\S]*?\}\);/m;
const finalLoopRepl = `activeData.items.forEach(item => {
          ...
          html += '<div class="p-6 rounded-lg bg-dark-card backdrop-blur-md border ' + extraClass + ' flex flex-col justify-between transition-all duration-300 relative overflow-hidden group hover:-translate-y-1 hover:shadow-[0_4px_20px_rgba(0,0,0,0.5)]">' +
              ...
              '<div class="flex flex-col gap-2 mt-8">' +
                ...
                '</a>' +
                (activeData.id.startsWith('minecraft') ? '<a href="https://panel.civizsicloudhosting.indevs.in" target="_blank" class="w-full py-3 px-4 rounded text-xs font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2 bg-dark-bg border border-dark-border hover:border-primary text-slate-300 hover:text-white">Panel <i data-lucide="server" class="h-3 w-3"></i></a>' : '') +
              '</div>' +
            '</div>';
        });`;

s = s.replace(finalLoopRegex, finalLoopRepl);

s = s.replace(/if \(activeData\.id === 'minecraft'\) \{/g, "if (activeData.id.startsWith('minecraft')) {");
